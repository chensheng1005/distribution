...
(
	...
	note nvarchar(50) comment '备注',
	foreign key(route) references Route(cId)
)default charset = utf8;
...
(
	...
	foreign key(provincesID) references Provinces(pId)
)default charset = utf8;
...
(
	...
	yprice decimal(18,0) comment '运输价格',
	note nvarchar(50) comment '备注',
	foreign key(pstartId) references Provinces(pId),
	foreign key(pmiddleId) references Provinces(pId),
	foreign key(pendId) references Provinces(pId),
	foreign key(cstartId) references City(cId),
	foreign key(cmiddleId) references City(cId),
	foreign key(cendId) references City(cId)
)default charset = utf8;
...
(
	...
	foreign key(aid) references Admin(adminID)
)default charset = utf8;

drop table if exists DisScope;
